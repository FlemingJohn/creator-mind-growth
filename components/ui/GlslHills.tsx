...
void main(void) {
  ...
  vec3 color = vec3(0.878, 0.722, 0.467);
  gl_FragColor = vec4(color, opacity);
}
`;

export function GlslHills({ cameraZ = 125, planeSize = 192, speed = 0.5 }: GlslHillsProps) {
  ...
  useEffect(
    function drawHills() {
      ...
      function drawFrame() {
        uniforms.time.value = uniforms.time.value + clock.getDelta() * speed;
